...
- 三方库目录结构
  ...
  ```shell
  tpc_c_cplusplus/thirdparty/marisa-trie    # 三方库marisa的目录结构如下
  ├── docs                                  # 三方库相关文档的文件夹
  ├── HPKBUILD                              # 构建脚本
  ├── SHA512SUM                             # 三方库校验文件
...
- 三方库头文件及生成的库
  ...
  ```shell
  marisa-trie/arm64-v8a   marisa-trie/armeabi-v7a
  ```
...
进入到构建目录执行`make check-TESTS -C tests`运行测试用例（arm64-v8a-build为构建64位的目录，armeabi-v7a-build为构建32位的目录）

&nbsp;![marisa_test](pic/marisa_test.png)
